...
/*
 * test 1 - set mosi high at beginning - use reg driver
 * test 2 - use bb driver - same as test 1
 * test 3 - modify the platform code to set it high
 */
...
void application_start(void)
{
    ...
    //spi power test - continuous send to measure power
    /*
    ...
    WPRINT_APP_INFO(("failcount=%f\n", (double)failcount));
    */

    //not power test section
    if (wiced_spi_transfer(&spi_device , message, nsegments)!=WICED_SUCCESS)
    {
    ...
    wiced_gpio_output_high(WICED_GPIO_21);
    wiced_gpio_deinit(WICED_GPIO_21);
    if( wiced_spi_init(&spi_device) != WICED_SUCCESS)
        WPRINT_APP_INFO(( "SPI device init fail\n" ));
    else
        WPRINT_APP_INFO(( "SPI device init success\n" ));
