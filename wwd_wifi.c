...
wwd_wifi_get_best_cca(int32_t band, uint32_t *channel, uint8_t *score, uint32_t duration)
{
    ...
    while (*chan_list != 0) {
        result = wwd_wifi_get_cca_for_channel(chan_list, duration, &tmp_score, 1);
        if (result == WWD_SUCCESS) {
...
{
    return wwd_wifi_set_ioctl_value( WLC_SET_ROAM_TRIGGER, (uint32_t)trigger_level, WWD_STA_INTERFACE );
}
...
wwd_result_t wwd_wifi_get_roam_trigger( int32_t* trigger_level )
{
    ...
    return wwd_wifi_get_ioctl_value( WLC_GET_ROAM_TRIGGER, (uint32_t*)trigger_level, WWD_STA_INTERFACE );
}

wwd_result_t wwd_wifi_set_roam_delta( int32_t trigger_delta )
{
    return wwd_wifi_set_ioctl_value( WLC_SET_ROAM_DELTA, (uint32_t)trigger_delta, WWD_STA_INTERFACE );
}
...
wwd_result_t wwd_wifi_get_roam_delta( int32_t* trigger_delta )
{
    ...
    return wwd_wifi_get_ioctl_value( WLC_GET_ROAM_DELTA, (uint32_t*)trigger_delta, WWD_STA_INTERFACE );
}
...
wwd_result_t wwd_wifi_get_cap( char *buf, uint16_t buflen, char *cap )
{
    ...
    if ( cap && strnstr( buf, buflen, cap, (uint16_t)strlen( cap ) ) == NULL )
    {
...
wwd_result_t wwd_join_mesh( const wiced_ssid_t* ssid, wwd_interface_t interface )
{
    wiced_buffer_t buffer;
    uint32_t*      data;
    char *jparam;
